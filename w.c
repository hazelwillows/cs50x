...
int check_word(string guess, int wordsize, int status[], string choice)
{
    ...
    for (int i = 0; i < wordsize; i++)
    {
        ...
        {
            if (guess[i] == choice[j] || guess[i]+32 == choice[j])
            {
                if (guess[i] == choice[i] || guess[i]+32 == choice[j])
                {
                    score +=2;
                    status[i] = 2;
                    break;
...
void print_word(string guess, int wordsize, int status[])
{
    ...
    for (int count = 0; count < wordsize; count++)
    {
        if (status[count] == 0)
        {
            printf(RED"%c"RESET, guess[count]);
        }
        else if (status[count] == 1)
        {
            printf(YELLOW"%c"RESET, guess[count]);
        }
        else
        {
            printf(GREEN"%c"RESET, guess[count]);
        }
    }
